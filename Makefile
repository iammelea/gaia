...
localnet-start: build-linux localnet-stop
	@if ! [ -f build/node0/gaiad/config/genesis.json ]; then docker run --rm -v $(CURDIR) /build:/gaiad:Z tendermint/gaiadnode testnet --keyring-backend=test --v 4 -o . --starting-ip-address 192.168.10.2 ; fi
	docker-compose up -d
